 ...
 */
public class WarnQuartzJob extends QuartzJobBean {
    private Logger logger = LoggerFactory.getLogger(getClass());

    private ExecutorService executorService = Executors.newSingleThreadExecutor();

    private String className = "warnSearchJob";
    private String MethodName = "run";

    @Override
    protected void executeInternal(JobExecutionContext context) throws JobExecutionException {
        SysSearchwarningConfig scheduleJob = (SysSearchwarningConfig) context.getMergedJobDataMap().get(SysSearchwarningConfig.JOB_WARN_KEY);
        ...
        scheduleJobLog.setBeanName(className);
        scheduleJobLog.setMethodName(MethodName);
        scheduleJobLog.setParams(scheduleJob.toString());
        try {
        ...
        try {
            ...
            WarnQuartzRunnable task = new WarnQuartzRunnable(className, MethodName, scheduleJob);

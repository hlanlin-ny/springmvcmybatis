    ...
    $().ready(function () {
        ...
        tables = $("#data-table")
                .dataTable({
                    ...
                    ajax: {
                        type: "post",
                        url: "${ctxPath}/admin/hospitaluser/queryList",
                    ...
                    <shiro:lacksPermission name="hospitaluser:batchdelete">
                    "order": [[1, "desc"]],
                    </shiro:lacksPermission>
                    <%--<shiro:hasPermission name="hospitaluser:batchdelete">--%>
                    <%--"order": [[2, "desc"]],--%>
                    <%--</shiro:hasPermission>--%>
                    columns: [
                        <shiro:hasPermission name="hospitaluser:batchdelete">
                        ...
                        <myshiro:hasAnyPermission permissions="hospitaluser:edit,hospitaluser:delete">
                        ,
                    ...
                    columnDefs: [
                        ...
                        <myshiro:hasAnyPermission permissions="hospitaluser:edit,hospitaluser:delete">
                        {
                            ...
                            <shiro:hasPermission name="hospitaluser:edit">
                            ...
                            <shiro:hasPermission name="hospitaluser:delete">
...
//                        });
        ...
        $("#btn-reset").on("click", function () {
            ...
            $("#extra-search-hospitalId").select2();
            $("#extra-search-hospitalId").find("option").eq(0).attr("selected", "selected");
            tables.fnDraw(false);
        });

        //添加
        $("#btn-add").on("click", function () {
            url = "${ctxPath}/admin/hospitaluser/add";
            $("#edit-modal-label").html("添加");
            $("#edit-form :input").val("");
            $("#areaId").val($("#extra-search-areaId").val());
            $("#disabled").find("option").eq(0).attr("selected", "selected");
            $("#hospitalId").find("option").eq(0).attr("selected", "selected");
        ...
        $("#data-table tbody").on("click", "#btn-edit-row", function () {
            url = "${ctxPath}/admin/hospitaluser/update";
        ...
        $("#btn-operate-confirm").on("click", function () {
            ...
            if (operateType == deleteToOperateConfirmFlag) {
                url = "${ctxPath}/admin/hospitaluser/" + operateTypeIds + "/delete";
                data = "";
            } else if (operateType == batchDeleteToOperateConfirmFlag) {
                url = "${ctxPath}/admin/hospitaluser/batch/delete";
                data = operateTypeIds;
            }
        ...
        $('#extra-search-hospitalId').append(item);
        $.get("${ctxPath}/admin/hospitalunitinformation/queryHospitalByAreaId", function (data) {
            $.each(data.result,
                    function (i, result) {
                        item = "<option value='" + result['id'] + "'>" + result['hospitalName'] + "</option>";
                        $('#hospitalId').append(item);
                        $('#extra-search-hospitalId').append(item);
                    })
        });

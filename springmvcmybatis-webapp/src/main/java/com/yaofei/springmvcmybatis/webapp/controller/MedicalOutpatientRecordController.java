 ...
 */
@Controller
@RequestMapping("/admin/medicaloutpatientrecord")
public class MedicalOutpatientRecordController extends BaseCRUDController<MedicalOutpatientRecord>  {
	@Autowired
	...
	public String export(HttpServletRequest request, HttpServletResponse response) throws ServletRequestBindingException, UnsupportedEncodingException {
		...
		if(finalFileName!=""){
			...
			//导出Excel对象
			workbook = medicalOutpatientRecordService.exportExcelInfo();
			OutputStream output;
			try {
	...
	public JsonResult updatetocommon(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		List<MedicalOutpatientRecord> list = medicalOutpatientRecordService.queryListAll();

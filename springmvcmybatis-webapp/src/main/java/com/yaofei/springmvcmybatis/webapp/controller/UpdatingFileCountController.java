 ...
 */
@Controller
@RequestMapping("/admin/updatingfilecount")
public class UpdatingFileCountController extends BaseCRUDController<UpdatingFileClass>  {
	@Autowired
	private UpdatingFileCountService updatingFileCountService;
	...
	public JsonResult queryCount(HttpServletRequest request, HttpServletResponse response) {
		...
		List<UpdatingFileClass> result = updatingFileCountService.queryCount(dataTablesRequest);
	...
	public JsonResult querySum(HttpServletRequest request, HttpServletResponse response) {
		//对请求参数解析，生成 排序条件、列搜索对象、全局搜索对象
		DataTablesRequest dataTablesRequest = DataTablesUtils.getDataTableRequest(request);
		List<UpdatingFileClass> result = updatingFileCountService.querySum(dataTablesRequest);
		return JsonResult .ok().addResult(result);
	}
}
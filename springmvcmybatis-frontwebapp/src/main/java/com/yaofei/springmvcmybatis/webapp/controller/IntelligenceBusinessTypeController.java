...
public class IntelligenceBusinessTypeController extends BaseCRUDController<IntelligenceBusinessType> {
    ...
    public JsonResult queryAllRightIntelligenceBusinessType(ServletRequest request) {
        ...
        Collections.sort(intelligenceBusinessTypes);

        List<TreeNode> treeNodes = new ArrayList<>();
        //添加顶级菜单
        TreeNode node = new TreeNode(Long.MIN_VALUE, Long.MIN_VALUE, "根", false, true);
        treeNodes.add(node);
        for (IntelligenceBusinessType intelligenceBusinessType : intelligenceBusinessTypes) {
            boolean hasParent = false;
            //如果treeNodes中没有包括父亲节点，则添加行政区划
            for (TreeNode treeNode : treeNodes) {
                if (treeNode.getId() == intelligenceBusinessType.getParentId()) {
                    hasParent = true;
                    break;
                }
            }
            if (hasParent) {
                ...
                //如果treeNodes中没有包括父亲节点，则添加行政区划
                for (TreeNode treeNode : treeNodes) {
                    if (treeNode.getId() == parnetId) {
                        hasArea = true;
                        break;
                    }
                }
                if (!hasArea) {
                    node = new TreeNode(parnetId, Long.MIN_VALUE, sysArea.getName(), false, true);
                    treeNodes.add(node);
                }
